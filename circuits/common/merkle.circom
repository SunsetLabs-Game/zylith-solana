...
/**
 * @title MerkleProof
 * @dev Verifies Merkle tree inclusion proofs for Sunset's state tree
 *
 * SECURITY CRITICAL:
 * This circuit proves that a leaf exists in a Merkle tree with a given root.
 * It is the foundation of all privacy guarantees in Sunset - if this is broken,
 * the entire protocol is broken.
 *
 * Design based on audited LeanIMTInclusionProof from Privacy Pools, adapted
 * for Sunset's fixed 20-level tree structure.
 *
 * Tree Properties:
 * - Fixed depth of 20 levels (supports 2^20 = ~1M leaves)
 * - Uses Poseidon hash for efficient on-chain verification and off-chain proof generation
 ...
 *
 * This pattern is used in most Sunset circuits to verify note ownership.
 *
