...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Mint proof generated — position and change commitments ready for wallet submission", body = MintResponse),
...
pub async fn shielded_mint(
    ...
    for (note, result) in [(&req.input_note_0, &input0), (&req.input_note_1, &input1)] {
    ...
    let predicted_root = predict_root(
        ...
        &[
            ...
            change_commitment_0.clone(),
            change_commitment_1.clone(),
        ],
    )
    .await?;

    tracing::info!("Shielded mint proof prepared for direct wallet submission");
