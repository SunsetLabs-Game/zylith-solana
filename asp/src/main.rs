...
use zylith_asp::relayer::SolanaRelayer;
...
async fn main() -> anyhow::Result<()> {
    ...
    let relayer = if config.enable_relayer {
        match SolanaRelayer::new(&config).await {
            Ok(r) => {
                tracing::info!("Solana relayer initialized");
                Some(Mutex::new(
                    Box::new(r) as Box<dyn zylith_asp::relayer::Relayer>
                ))
            }
            Err(e) => {
        ...
        tracing::info!("Solana relayer disabled; running in proof-only mode");
        None
    };
    ...
    if config.enable_event_sync {
        ...
        tokio::spawn(async move {
            zylith_asp::sync::solana::start_event_sync(sync_state, poll_interval).await;
        });
    } else {
        tracing::info!("Solana event sync disabled");
    }
