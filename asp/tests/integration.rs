...
// ---------------------------------------------------------------------------
// MockRelayer — returns configurable tx hashes, no real on-chain calls
// ---------------------------------------------------------------------------
...
async fn test_deposit_two_sequential() {
    ...
    assert_eq!(body2["leaf_index"], 0);
    let root2 = body2["root"].as_str().unwrap().to_string();

    // The prepared root reflects the requested commitment against the same empty DB snapshot.
