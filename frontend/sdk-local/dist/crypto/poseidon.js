/**
 * BN128 Poseidon hash wrapper using circomlibjs.
 *
 * This uses the BN254 scalar field (~2^254), NOT the Stark field (~2^251).
 * All Zylith commitments, nullifier hashes, and Merkle tree operations
 * use this hash function to match the Circom circuits.
 */
import { buildPoseidon } from "circomlibjs";
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let _poseidon = null;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let _F = null;
/** Initialize the Poseidon hash function. Must be called before any hashing. */
export async function initPoseidon() {
    if (!_poseidon) {
        console.log("[Sunset SDK] Initializing Poseidon...");
        try {
            // buildPoseidon() can hang if WASM fails. 
            // We add a 10s timeout to avoid permanent UI hang.
            const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error("Poseidon initialization timed out (10s)")), 10000));
            _poseidon = await Promise.race([buildPoseidon(), timeout]);
            _F = _poseidon.F;
            console.log("[Sunset SDK] Poseidon initialized successfully");
        }
        catch (e) {
            console.error("[Sunset SDK] Poseidon initialization failed:", e);
            throw e;
        }
    }
}
/** Hash an array of inputs using Poseidon. Returns a decimal string. */
export function hash(inputs) {
    if (!_poseidon)
        throw new Error("Call initPoseidon() first");
    const result = _poseidon(inputs.map((x) => _F.e(x)));
    return _F.toString(result, 10);
}
/** Check if Poseidon has been initialized */
export function isInitialized() {
    return _poseidon !== null;
}
//# sourceMappingURL=poseidon.js.map