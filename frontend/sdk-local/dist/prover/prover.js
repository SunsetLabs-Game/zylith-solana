    ...
    async generateProof(circuit, inputs) {
        ...
        for (const path of [
            ...
            await access(path).catch(() => {
                throw new Error(`Missing circuit artifact: ${path}. Run 'npm run copy-artifacts' first.`);
            });
        }
        // Generate witness + proof
        const { proof, publicSignals } = await snarkjs.groth16.fullProve(inputs, artifacts.wasmPath, artifacts.zkeyPath);
        // Local verification
