 ...
 */
function getAmount0Delta(sqrtPriceA, sqrtPriceB, liquidity) {
    const [sqrtLower, sqrtUpper] = sqrtPriceA < sqrtPriceB ? [sqrtPriceA, sqrtPriceB] : [sqrtPriceB, sqrtPriceA];
    if (sqrtUpper === 0n || sqrtLower === 0n)
        return 0n;
 ...
 */
function getAmount1Delta(sqrtPriceA, sqrtPriceB, liquidity) {
    const [sqrtLower, sqrtUpper] = sqrtPriceA < sqrtPriceB ? [sqrtPriceA, sqrtPriceB] : [sqrtPriceB, sqrtPriceA];
 ...
 */
export function estimateSwapOutput(sqrtPrice, amountIn, zeroForOne, feePips) {
    if (amountIn === 0n || sqrtPrice === 0n)
        return 0n;
 ...
 */
export function estimateSwapOutputSafe(sqrtPrice, amountIn, zeroForOne, feePips, slippageBps = 100) {
 ...
 * @returns Estimated output amount
 */
export function estimateSwapOutputConstantProduct(reserveIn, reserveOut, amountIn, feePips) {
 ...
 * @returns Estimated output amount reduced by slippage buffer
 */
export function estimateSwapOutputConstantProductSafe(reserveIn, reserveOut, amountIn, feePips, slippageBps = 100) {
 ...
 */
export function getAmountsForBurn(sqrtPrice, tickLower, tickUpper, liquidity) {
