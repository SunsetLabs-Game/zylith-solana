...
import { generateSwapInputs } from "../prover/inputs/swap.js";
export async function swap(params, asp, noteManager) {
    const notes = noteManager.getAllNotes();
    const inputNote = notes.find((n) => n.commitment === params.inputNoteCommitment && !n.spent);
    if (!inputNote)
        throw new Error("Input note not found or already spent");
    ...
    if (params.useAspProver) {
        ...
        const proofRes = await asp.getTreePath(inputNote.leafIndex);
        // 2. Generate circuit inputs
        const circuitInputs = generateSwapInputs({
            inputNote: {
                secret: inputNote.secret,
                nullifier: inputNote.nullifier,
                balance: BigInt(inputNote.amount),
                token: params.tokenIn,
                merkleProof: {
                    root: proofRes.root,
            ...
            changeNote: { secret: changeSecret, nullifier: changeNullifier },
        ...
        const { proof, publicSignals } = await prover.generateProof("swap", circuitInputs);
        // 4. Format proof for Solana
        solanaProof = formatProofForSolana(proof);
        // Mock response for now until Solana TX relayer is updated
        response = {
            new_commitment: circuitInputs.newCommitment,
            change_commitment: circuitInputs.changeSecret, // This is a bug in the old SDK that returned the secret? Wait, swap.circom returns changeCommitment as output!
            amount_out: params.expectedAmountOut.toString(),
            amount_change: (BigInt(inputNote.amount) - params.amountIn).toString(),
            calldata: [],
            final_root: proofRes.root,
        };
        // snarkjs returns public signals in order.
        // For swap: [root, nullifierHash, newCommitment, tokenIn, tokenOut, amountIn, amountOutMin, changeCommitment]
        response.change_commitment = publicSignals[7];
    }
    ...
    if (actualChangeAmount > 0n && response.change_commitment && response.change_commitment !== "0") {
        ...
        if (changeNote)
            noteManager.markSpent(changeNote.nullifierHash);
    }
